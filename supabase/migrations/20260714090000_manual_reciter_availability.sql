-- Manual, session-only reciter availability.
-- The stored value is the current effective advertisement, not the reciter's
-- switch preference. Existing reciters intentionally start unavailable.
ALTER TABLE public.reciter_profiles
ADD COLUMN IF NOT EXISTS is_available boolean NOT NULL DEFAULT false;

-- Complete the constraint/default if a prior partial application created the
-- column without the final shape.
