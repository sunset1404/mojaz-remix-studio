...
-- 1. payment_invoice_metadata already created in an earlier migration.
-- Keep this block guarded to avoid duplicate table/policy/index errors on fresh setups.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'payment_invoice_metadata'
  ) THEN
